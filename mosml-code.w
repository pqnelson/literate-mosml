...
value sml_getrutime (value v) /* ML */
{
  ...
  /*
   ...
   Perhaps, win32 enables |sysTime| and |usrTime| to be measured
   in an accurate way...
...
value sml_mkdir(value path)          /* ML */
{
  ...
  /* Unlike Unix and DJ GPP, the path may contain a drive letter,
   and must contain '\BS' rather than '/'. */
...
@ Turn an ML value into an externalized ML value (a string),
\textit{\'{a} la} |extern.c|
...
@ Turn an externalized ML value (a string) into an ML value,
\textit{\'{a} la} |intern.c|
