...
Algorithm:
  ...
  1-  If argument 0 is a valid byte-code file that does not start with \texttt{\#!},
      then we are in case 3 and we pass the same command line to the
      Caml Light program.
      
  2-  In all other cases, we parse the command line as:\par
        \centerline{(whatever) [options] bytecode args...}\par
      and we strip "(whatever) [options]" from the command line.

@ @(main.c@>=
#ifdef WIN32
#define main caml_main(int argc, char * argv[])
...
int main(int argc, char * argv[])
{
  ...
  @<Restore default `Ctrl-C' behaviour on windows@>@;
...
#endif  /* |defined(DIRECT_JUMP) && defined(THREADED)| */
...
#endif /* |defined(__CYGWIN__) || defined(hpux) || defined(WIN32)| */
      free(exnmsg);
      fatal_error(buf);
    }
  }
...
@ The fine documentation from:

{\tt https://learn.microsoft.com/en-us/windows/console/setconsolectrlhandler}

\noindent%
states:

{\advance\leftskip3pc\advance\rightskip3pc
If the \\{HandlerRoutine} parameter is |NULL|, a |TRUE| value causes the calling process to ignore CTRL+C input, and a |FALSE| value restores normal processing of CTRL+C input. This attribute of ignoring or processing CTRL+C is inherited by child processes.\par}
