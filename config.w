...
@
On Windows, we need care about |extern| being inadequate (we need to
also include an extra Windows-specific attribute). This forces us to
introduce a \texttt{EXTERN} macro, which is |extern| as we expect it
to be.

@f EXTERN extern
@(config.h@>=
#ifdef WIN32
@<|EXTERN| macro for Windows@>@;
#else
#define EXTERN extern
#endif

@ @<|EXTERN| macro for Windows@>=
#ifdef CAMLRT /* CaML runtime [default] */
...
@<Configuration for 16-bit machines@> @;
#else
#ifdef SMALL                   /* Scaled-down parameters for small memory */

@<Memory configuration for ``small memory''@> @;
